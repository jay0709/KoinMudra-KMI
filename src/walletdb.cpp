...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Koinmudra with fAggressive=true to
    // get as much data as possible.
    // Rewrite koinmudrad data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> koinmudradData;
    bool allOK = dbenv.Koinmudra(newFilename, true, koinmudradData);
    if (koinmudradData.empty()) {
        LogPrintf("Koinmudra(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Koinmudra(aggressive) found %u records\n", koinmudradData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, koinmudradData) {
        if (fOnlyKeys) {
